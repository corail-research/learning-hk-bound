...
void BB::filter_and_force(std::vector<Edge*>* toRemove, std::vector<Edge*>* toForce, bool *canStopBranching) {
	clock_t endTime=clock();

	int tmm =(int) ( (endTime-startTime)/(60.0*(double)CLOCKS_PER_SEC) );
...
void BB::filter_and_force_ap(std::vector<Edge*>* toRemove, std::vector<Edge*>* toForce, bool *canStopBranching) {
	...
	int tmm =(int) ( (endTime-startTime)/(60.0*CLOCKS_PER_SEC) );
	
	if(tmm >= 3000) {
		cerr<<"Stopping after "<<tmm<<"."<<endl;
		printEndInfos(false);
		//cerr<<"Nb BB nodes : "<<nbNode<<endl;
		exit(1);
	}
