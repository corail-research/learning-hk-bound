...
void HeldKarp::computeBound(double upperBound, bool *tourFound, bool *canStopBranching, float *HKbound_fnf, float factors[]) {
...
*/
	...
	*tourFound=false;
	...
	//cout<<"l="<<l<<endl;
		for(int k=1; k< nbIter+1; k++) {
			...
			for(std::vector<Pointeur*>::iterator it = pointeurs.begin(); it!= pointeurs.end() ; ++it) {
				...
				//cout<<"node "<<index<<" degre "<<(*it)->degre<<" penalite "<<penalite[index]<<endl;
	...
	for(std::vector<Pointeur*>::iterator it = pointeurs.begin(); it!= pointeurs.end() ; ++it) {
		...
		factors[i] = bestPenalites[index]; //added
		i++;//added
		totalPenalite+=penalite[index];
		cout << penalite[index] << endl;
	}
	PrimMieux(&canContainTour);
	if(!canContainTour) {
		HKBound=std::numeric_limits<double>::infinity();
...
/*
pair<bool,bool> HeldKarp::computeBound2(double upperBound) {
	cout <<endl<<"Compute bound version 2"<<endl;
	//cout << "old bound "<<HKBound<<endl;
	...
	//double step=0.0;
	double oldBound = 0.0;
	double oldPenalite[graphe->getMaxSize()];
	double oldDegre[graphe->getMaxSize()];
	double oldViolation=0.0;
	
	//index1node=0;
	
	bool connexe=PrimMieux();
	if(!connexe) {
		HKBound=std::numeric_limits<double>::infinity();
	...
	//HKBound=oneTreeWeight;
	
	double tPenalite=0.0;
	for(std::vector<Pointeur*>::iterator it = pointeurs.begin(); it!= pointeurs.end() ; ++it) {
			oldViolation+=(2-(*it)->degre)*(2-(*it)->degre);
			oldDegre[(*it)->nodeRef->getIndex()]=	(*it)->degre;
			tPenalite+=penalite[(*it)->nodeRef->getIndex()];
	}
	
	HKBound=oneTreeWeight-2*tPenalite;
	cout<<"first bound :"<<HKBound<<endl;
	//oldBound=HKBound;
	int nbDescenteAlpha=0;
	int nbIteration=0;
	int nbChangementDegre=0;
	int nbCount1=0;
	while(true) {
	//cout<<" l="<<l<<endl;
		bool first=true;
		...
		while(true) {
			...
			//si on a un tour, on arrete
			if(tour) {
				cout <<"tour found in HK!"<<endl;
				cout<<"Minimum tour weight : "<<HKBound<<endl;
			...
			//calcul du pas
			double step=alpha*(upperBound-HKBound)/violation;
			

			
			sumStep+=step;
			
			if(HKBound > realBound) {
				cout<<"HKBound > realBound   "<<HKBound<< endl;
			...
			//si le pas est trop petit, on arrete.
			
			if(step< tSmall) {
				//cout<<"step < tsmall"<<endl;
				//cout<<"nb descente alpha "<<nbDescenteAlpha<<endl;
				//cout <<"nb de while à 1 iteration :  "<<nbCount1<<endl;
				cout<<"nb iteration "<<nbIteration<<endl;
				cout<<"Held and Karp bound : "<<HKBound<<endl;
				computed=true;
				//PrimMieux();
				return pair<bool,bool>(true,false);
			}
			...
			for(std::vector<Pointeur*>::iterator it = pointeurs.begin(); it!= pointeurs.end() ; ++it) {
				...
				int index = (*it)->nodeRef->getIndex();
				//mise à jour de la pénalité
				oldPenalite[index]=penalite[index];
				//penalite[index]+=step*(0.7*pointeurs[index]->degre+0.3*oldDegre[index]-2);
				penalite[index]+=step*(pointeurs[index]->degre-2);
			...
			HKBound= oneTreeWeight-2*totalPenalite;
			//computeBound4(upperBound);
			
			
			
			if(HKBound<oldBound) {
				cout<<endl<<"Descente !!!!!!!!!!!!!"<<endl;
				cout<<"HKBound "<<HKBound<<endl;
				cout <<"oldBound" <<oldBound<<endl;
				double sumP=0.0;
				for(std::vector<Pointeur*>::iterator it = pointeurs.begin(); it!= pointeurs.end() ; ++it) {
					...
					int index = (*it)->nodeRef->getIndex();
					//mise à jour de la pénalité
					penalite[index]=oldPenalite[index];
					(*it)->degre=oldDegre[index];
					sumP+=penalite[index];
				}	
				PrimMieux();
				HKBound=oneTreeWeight-2*sumP;
...
pair<bool,bool> HeldKarp::computeBound3(double upperBound) {
	cout <<endl<<"Compute bound version 3"<<endl;
	...
	//cout<<"l="<<l<<endl;
		for(int k=1; k< period; k++) {
			...
			if(k==period-1 && HKBound > oldBound) {
				period/=beta;
			}
			
		//	cout<<"total penalite : "<<totalPenalite;
			//			cout<<"     violation "<<violation<<" count "<<count;
			//cout<<"         HK bound : "<<HKBound<<endl;

			//si on a un tour, on arrete
			if(tour) {
				cout <<"Minimum tour found !"<<endl;
				cout<<"Minimum tour weight : "<<HKBound<<endl;
				computed=true;
				return pair<bool,bool>(true,true);
			}
		...
		//	step=alpha*(upperBound-HKBound)/violation;
			...
			//if(step < tSmall) {
			//	cout<<"nb d'iterations : "<<nbIteration<<endl;
			//	cout<<"Held and Karp bound : "<<HKBound<<"      totalPenalite : "<<totalPenalite<< endl;
			//	cout<<penalite[0]<<endl;
			//	computed=true;
			//	return pair<bool,bool>(true,false);
			//}
